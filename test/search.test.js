...
import { mercadoLivreImageFromId, normalizeImageUrl, upgradeMercadoLivreImage } from '../src/services/search/imageValidation.service.js';
...
test('dedupeProducts removes duplicates', () => {
  const products = [
    normalizeProduct({ id: '1', title: 'A', url: 'https://x.test/a' }, 'mock'),
    normalizeProduct({ id: '2', title: 'A', url: 'https://x.test/a' }, 'mock')
  ];
...
test('Mercado Livre thumbnail_id resolves to high density image URL', () => {
  const image = mercadoLivreImageFromId('123456-MLB78901234567_092024');
  assert.equal(image, 'https://http2.mlstatic.com/D_NQ_NP_2X_123456-MLB78901234567_092024-F.webp');
});

test('Mercado Livre image upgrade keeps existing 2X marker once', () => {
  const image = upgradeMercadoLivreImage('http://http2.mlstatic.com/D_NQ_NP_2X_123456-MLB78901234567_092024-O.jpg');
  assert.equal(image, 'https://http2.mlstatic.com/D_NQ_NP_2X_123456-MLB78901234567_092024-F.webp');
});

test('search falls back when no real sources return', async () => {
  ...
  assert.equal(result.meta.fallbackResults > 0, true);
});

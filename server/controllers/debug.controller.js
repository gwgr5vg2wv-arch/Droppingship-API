...
export async function checkNetwork(req, res, next) {
  ...
  try {
    ...
    saveDebugInfo({ lastUrl: url, lastError: '', lastResponseTimeMs: elapsed, lastRequestAt: new Date().toISOString(), lastMode: process.env.INTEGRATION_MODE || 'mock' });
    return res.json({ online: response.status < 400, status: response.status, responseTimeMs: elapsed, url });
  } catch (error) {
    const elapsed = Date.now() - start;
    saveDebugInfo({ lastUrl: url, lastError: error.message, lastResponseTimeMs: elapsed, lastRequestAt: new Date().toISOString(), lastMode: process.env.INTEGRATION_MODE || 'mock' });
...
export async function checkMercadoLivre(req, res, next) {
  ...
  try {
    ...
    const elapsed = Date.now() - start;
    saveDebugInfo({ lastUrl: url, lastError: '', lastResponseTimeMs: elapsed, lastRequestAt: new Date().toISOString(), lastResultCount: response.data?.results?.length || 0, lastMode: process.env.INTEGRATION_MODE || 'mock' });

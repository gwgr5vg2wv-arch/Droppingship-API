...
export function scoreProduct(product = {}) {
  const totalCost = calculateTotalCost(product.supplierPrice, product.shippingPrice);
  const suggestedPrice = product.suggestedPrice && product.suggestedPrice >= totalCost
    ? money(product.suggestedPrice)
    : calculateSuggestedPrice({ supplierPrice: product.supplierPrice, shippingPrice: product.shippingPrice });
  const estimatedProfit = calculateProfit({ supplierPrice: product.supplierPrice, shippingPrice: product.shippingPrice, suggestedPrice });
  const roi = calculateRoi(estimatedProfit, totalCost);
  const trendScore = product.trendScore || calculateTrendScore(product);
  const competitionLevel = product.competitionLevel || calculateCompetitionLevel(product);
  const riskLevel = product.riskLevel || calculateRiskLevel({ ...product, roi });
  const opportunityScore = product.opportunityScore || calculateOpportunityScore({ ...product, roi, trendScore, competitionLevel, riskLevel });

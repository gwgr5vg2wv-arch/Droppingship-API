...
import { enabledRealProviders, providerStatus } from './sourceRegistry.js';
...
export async function searchProducts(query, options = {}) {
  ...
  const disabled = (await providerStatus()).filter((item) => item.disabled);
  ...
  const response = {
    ...
    meta: {
      ...
      realResults: products.length,
      fallbackResults: 0,
      durationMs: Date.now() - started,
      cacheHit: false,
      providers: products.length ? providerMeta : [],
      message: products.length ? null : 'Nenhuma fonte real retornou produtos agora.'
    }
  };

  console.log(`[SEARCH] completed real=${response.meta.realResults} duration=${response.meta.durationMs}ms`);
...
export async function searchTrends(options = {}) {
  ...
  return {
    ...
    summary: {
      ...
      topSource: sourceCounts[0]?.name || null,
      message: result.products.length ? null : 'Ainda nao ha historico real suficiente para calcular tendencia.'
    }
  };
}

...
import { mercadoLivreImageFromId, upgradeMercadoLivreImage } from '../imageValidation.service.js';
  ...
  async searchProducts(query, options = {}) {
    ...
    try {
      ...
      const products = (data.results || []).map((item) => {
        const image = mercadoLivreImageFromId(item.thumbnail_id) || upgradeMercadoLivreImage(item.secure_thumbnail || item.thumbnail);
        return {
        id: `ML-${item.id}`,
        externalId: item.id,
        title: item.title,
        ...
        images: [image, item.secure_thumbnail || item.thumbnail].filter(Boolean),

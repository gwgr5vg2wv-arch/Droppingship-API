...
async function searchRealProducts() {
  ...
  try {
    ...
    const data = await window.DroppingshipApi.searchProducts(query, { limit: 20, source: sources[0] }, { signal: activeSearchController.signal });
    state.products = data.products || [];
    state.productsFallback = data.meta?.realResults ? null : 'no_real_sources';
    renderSourceStatus(Object.fromEntries((data.meta?.providers || []).map((item) => [item.name, {
      ok: item.ok,
      mode: item.disabled ? 'disabled' : (item.ok ? 'real' : 'unavailable'),
      fallbackUsed: false,
      fallbackReason: item.message
    }])));
    renderProducts();
    toast('Busca concluida com ranking por oportunidade.');
...
async function loadDashboard() {
  try {
    ...
    const cards = [
      ...
      ['Pedidos reais', summary.simulatedOrders],
...
async function loadOrders() {
  try {
    ...
    document.getElementById('orders-list').innerHTML = state.orders.map(orderCard).join('') || empty('Nenhum pedido real encontrado.');
  } catch (error) {
...
async function loadFinance() {
  try {
    ...
    const cards = [
      ...
      ['Taxas marketplace', money.format((data.marketplaceFees ?? 0))]
    ];
...
function renderProducts() {
  ...
  document.getElementById('product-results').innerHTML = notice + (state.products.map(productCard).join('') || empty('Nenhum produto real foi encontrado. Tente outro termo ou conecte uma fonte de produtos.'));
}
      ...
      <div class="product-body">
        <div class="product-head">
          <div>
            ...
            ${badge(modeLabel(product), product.mode || product.source || 'real')}
        ...
        <div class="stats-grid">
          ${miniStat('Fornecedor', moneyOrDash(product.supplierPrice))}
          ${miniStat('Frete', moneyOrDash(product.shippingPrice ?? product.shippingCost))}
          ${miniStat('Lucro', moneyOrDash(product.estimatedProfit ?? product.profit))}
          ${miniStat('ROI', product.roi == null ? '-' : `${safeNumber(product.roi)}%`)}
          ${miniStat('Score', displayValue(product.score))}
          ${miniStat('Rating', displayValue(product.rating))}
          ${miniStat('Vendidos', displayValue(product.soldQuantity ?? product.sold))}
          ${miniStat('Entrega', product.deliveryDays == null ? '-' : `${safeNumber(product.deliveryDays)} dias`)}
        </div>
...
function renderTrendCategories(groups = []) {
  ...
  document.getElementById('trend-category-strip').innerHTML = categories.map(([key, label]) => {
    ...
    return `<button type="button" class="category-chip" onclick="setTrendCategory('${key}')">${label}</button>`;
  }).join('');
}
...
function renderTrends() {
  ...
  document.getElementById('trends-list').innerHTML = notice + (ranked.map((item) => productCard(item, 'trends')).join('') || empty('Ainda nao ha historico real suficiente para calcular tendencia.'));
}
  ...
  return `
    <article class="integration-card">
      <div>
        ...
        <p>Modo: ${item.mode} | Configuracao: ${item.configured ? 'credenciais detectadas' : 'Esta fonte ainda nao esta conectada'}</p>
...
window.syncOrders = async () => {
  try {
    ...
    state.orders = data.orders || [];
    document.getElementById('orders-list').innerHTML = state.orders.map(orderCard).join('') || empty('Nenhum pedido real encontrado.');
    toast(data.fallbackUsed ? 'Pedidos sincronizados.' : 'Pedidos sincronizados.');
  } catch (error) {
    toast(error.message);
  }
};
...
function modeLabel(product) {
  ...
  if (product.mode === 'public') return 'Publico permitido';
  if (product.mode === 'unavailable') return 'Indisponivel';
  return 'Real';
}
...
function renderSourceStatus(sources) {
  ...
  container.innerHTML = Object.entries(sources).map(([source, info]) => `
    <article class="source-status">
      ...
      <strong>${info.ok ? 'Busca oficial ativa' : 'Esta fonte nao respondeu agora'}</strong>
      <span>${info.fallbackUsed ? 'Os demais resultados reais foram mantidos.' : 'Fonte consultada sem expor credenciais.'}</span>
    </article>
  `).join('');
}
...
function resolveProductImage(product = {}) {
  return localAsset(product.image || product.thumbnail);
}

function productImageMarkup(product = {}) {
  const image = resolveProductImage(product);
  if (!image) return '<div class="no-image">Sem imagem real</div>';
  return `<span class="image-skeleton"></span><img src="${image}" loading="lazy" decoding="async" referrerpolicy="no-referrer" alt="${escapeHtml(product.title)}" onerror="handleProductImageError(this)">`;
}
...
window.handleProductImageError = (img) => {
  ...
  img.remove();
  if (wrapper && !wrapper.querySelector('.no-image')) {
    wrapper.insertAdjacentHTML('beforeend', '<div class="no-image">Imagem indisponivel</div>');
  }
};
...
function fallbackNotice() {
  return '<div class="fallback-notice"><span>i</span>Nenhuma fonte real retornou produtos agora.</div>';
}
...
function displayValue(value) {
  return value === null || value === undefined || value === '' ? '-' : safeNumber(value);
}

function moneyOrDash(value) {
  return value === null || value === undefined || value === '' ? '-' : money.format(safeNumber(value));
}
